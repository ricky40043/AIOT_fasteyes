...
def get_electrostatic_devices(db: Session, group_id: int):
    return db.query(device).filter(device.device_model_id == DeviceType.electrostatic.value, device.group_id == group_id).all()
